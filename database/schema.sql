...
CREATE TABLE races (
    ...
    stat_bonuses JSON, -- {"str": 2, "dex": 1, "agi": 1, "con": 2, "int": 0, "per": 0, "will": 0}
...
CREATE TABLE classes (
    ...
    description TEXT,
    primary_stats JSON, -- ["STR", "CON"] para Warrior, ["INT", "WILL"] para Mage
    stat_growth JSON, -- {"str": 3.5, "con": 2.5, "int": 1.0, ...}
...
CREATE TABLE skills (
    ...
    class_id INT, -- NULL = skill universal
    ...
    formula VARCHAR(255), -- "BASE_DAMAGE * (1 + STR * 0.05) * (1 + skill_level * 0.1)"
    base_value INT DEFAULT 0,
    scaling_stat ENUM('STR', 'DEX', 'AGI', 'CON', 'INT', 'PER', 'WILL') NULL,
...
CREATE TABLE dungeons (
    ...
    completion_bonus JSON, -- {"items": [{"id": 1, "chance": 100}], "premium_currency": 10}
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
);
...
CREATE TABLE dungeon_rooms (
    ...
    environmental_effects JSON, -- {"visibility": "dark", "bonus": "fire_damage_+20%"}
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE enemies (
    ...
    -- Resistances
    resistances JSON, -- {"fire": 0.8, "ice": 1.2, "physical": 0.9}
    ...
    skill_weights JSON, -- [{"skill_id": 1, "weight": 50}, {"skill_id": 2, "weight": 30}]
...
CREATE TABLE items (
    ...
    -- Consumable effects
    consumable_effect JSON, -- {"heal_hp": 100, "restore_mp": 50, "buff": {"str": 5, "duration": 300}}
    
    -- Stacking & Trading
    max_stack INT DEFAULT 1,
    is_tradable BOOLEAN DEFAULT TRUE,
    is_sellable BOOLEAN DEFAULT TRUE,
    base_value INT DEFAULT 0, -- Valor en oro
...
CREATE TABLE loot_tables (
    ...
    guaranteed_drops JSON, -- [{"item_id": 1, "quantity": 1}]
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE loot_table_entries (
    ...
    weight INT DEFAULT 100, -- Para loot aleatorio ponderado
...
CREATE TABLE player_inventory (
    ...
    is_stashed BOOLEAN DEFAULT FALSE, -- En almacén vs inventario activo
    ...
    INDEX idx_equipped (player_id, equipped) WHERE equipped = TRUE
);
...
CREATE TABLE parties (
    ...
    required_item_id INT NULL, -- Requisito de ítem para entrar
...
CREATE TABLE party_members (
    ...
    pending_action JSON NULL, -- Acción programada para el próximo tick
    action_cooldown JSON, -- {"skill_id": 123, "cooldown_until_tick": 15}
...
CREATE TABLE combat_logs (
    ...
    INDEX idx_player_combat (actor_id, created_at) WHERE actor_type = 'player'
);
...
CREATE TABLE player_tick_actions (
    ...
    -- Priority & timing
    priority INT DEFAULT 50, -- Lower = executes first
...
CREATE TABLE guilds (
    ...
    officer_ids JSON, -- [player_id1, player_id2, ...]
    ...
    item_bank JSON, -- {"items": [{"id": 1, "quantity": 5}], "capacity": 100}
    
    -- Settings
    max_members INT DEFAULT 50,
    recruitment_status ENUM('Open', 'InviteOnly', 'Closed') DEFAULT 'Open',
    requirements JSON, -- {"min_level": 10, "required_class_ids": [1, 2]}
    
    -- Perks & bonuses
    guild_perks JSON, -- {"exp_bonus": 0.05, "gold_find": 0.10}
...
CREATE TABLE guild_members (
    ...
    -- Permissions
    permissions JSON, -- {"can_invite": true, "can_kick": false, ...}
...
CREATE TABLE achievements (
    ...
    criteria JSON NOT NULL, -- {"stat": "dungeons_completed", "threshold": 10}
...
CREATE TABLE player_achievements (
    ...
    -- Progress tracking
    progress JSON, -- {"current": 5, "max": 10}
...
CREATE TABLE maintenance_schedule (
    ...
    affected_services JSON, -- ["auction_house", "combat", "dungeons"]
    announcements JSON,
    created_by INT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
BEGIN
    ...
    SET NEW.max_hp = race_base_hp + (NEW.con * class_hp_per_con);
    ...
    SET NEW.max_mp = (NEW.will * class_mp_per_will);
...
SELECT 
    ...
    COUNT(DISTINCT p.id) as parties_completed,
